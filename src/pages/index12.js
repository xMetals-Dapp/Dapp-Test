...
import xPT_NFT_Contract from "components/xPT_NFT";
...
const setContract = async () => {
	...
	let stableAllowance = await tContract.allowance(address,"0x6492f7BEBe0C60E12ad9F60be39aFd7B6AA651Eb");
...
const findMetalAllowance = async () => {
	...
	let temp_contract = xPTContract(provider);
	let metalAllowance = await temp_contract.allowance(address,"0x6492f7BEBe0C60E12ad9F60be39aFd7B6AA651Eb");
...
let address = getTheAddress();
//sets the allowance and balance of the stable coin 
let stableAllowance = await contract_withProvider.allowance(address,"0x6492f7BEBe0C60E12ad9F60be39aFd7B6AA651Eb");
setStableAllowance(stableAllowance/(10 ** 6));
...
try{
	...
	let resp = await temp.approve("0x6492f7BEBe0C60E12ad9F60be39aFd7B6AA651Eb",newPrice); //approve the spending amount to the metal contract for minting
	console.log(resp);
	let newAmount = realAmount * (10 ** 6);
	let temp2 = (xPTContract(provider)).connect(signer);
	let resp2 = await temp2.approve("0x6492f7BEBe0C60E12ad9F60be39aFd7B6AA651Eb",newAmount);
	console.log(resp2);
...
try{
	...
	let provider = getProvider();
	let temp = (xPT_NFT_Contract(provider)).connect(signer);
	...
	return <Theme theme={theme}>
		...
		>
			...
			<Box
				...
				align-items="center"
			>
				<Image src="https://uploads.quarkly.io/63e1507af2f031001fb80741/images/Platinum.jpg?v=2023-04-21T18:20:03.924Z" display="block" width="100%" border-radius="30px 30px 0px 00px" />
				<Box />
				<Box display="flex" flex-direction="column" align-items="center" justify-content="center">
					...
					>
						...
						<br />
						<br />
						You will receive a 1 troy ounce (31.1035g) Platinum bar from a LBMA/LPPM approved manufacturer, guaranteed to be .9995 pure. The bar(s) come in sealed/secure packaging and shipping is insured, with a tracking number being provided. Brands are likely to be Valcambi, Argor-Heraeus, Baird or PAMP.
					</Text>
					<Box height="40%" />
				</Box>
			</Box>
