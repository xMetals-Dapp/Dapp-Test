...
import xPD_NFT_Contract from "components/xPD_NFT";
...
export default (() => {
	...
	const setContract = async () => {
		...
		let stableAllowance = await tContract.allowance(address,"0x0270DAB4c23a589112862a42658F1837ac990BE8");
	...
	const findMetalAllowance = async () => {
		...
		let metalAllowance = await temp_contract.allowance(address,"0x0270DAB4c23a589112862a42658F1837ac990BE8");
	...
	let address = getTheAddress();
	//sets the allowance and balance of the stable coin 
	let stableAllowance = await contract_withProvider.allowance(address,"0x0270DAB4c23a589112862a42658F1837ac990BE8");
	setStableAllowance(stableAllowance/(10 ** 6));
	...
	try{
		...
		let resp = await temp.approve("0x0270DAB4c23a589112862a42658F1837ac990BE8",newPrice); //approve the spending amount to the metal contract for minting
		console.log(resp);
		let newAmount = realAmount * (10 ** 6);
		let temp2 = (xPDContract(provider)).connect(signer);
		let resp2 = await temp2.approve("0x0270DAB4c23a589112862a42658F1837ac990BE8",newAmount);
		console.log(resp2);
	...
	try{
		...
		let provider = getProvider();
		let temp = (xPD_NFT_Contract(provider)).connect(signer);
		...
		return <Theme theme={theme}>
			<GlobalQuarklyPageStyles pageUrl={"index1211"} />
			<Helmet>
			<title>
					xMetals | Dapp
				</title>
			...
			>
				...
				<Box
					...
					align-items="center"
				>
					<Image src="https://uploads.quarkly.io/63e1507af2f031001fb80741/images/Palladiuum.jpg?v=2023-04-21T18:20:03.929Z" display="block" width="100%" border-radius="30px 30px 0px 00px" />
					<Box />
					<Box display="flex" flex-direction="column" align-items="center" justify-content="center">
						...
						>
							...
							<br />
							<br />
							You will receive a 1 troy ounce (31.1035g) Palladium bar from a LBMA/LPPM approved manufacturer, guaranteed to be .9995 pure. The bar(s) come in sealed/secure packaging and shipping is insured, with a tracking number being provided. Brands are likely to be Valcambi, Argor-Heraeus, Baird or PAMP.
